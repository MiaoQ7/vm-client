...
220 垄断时间这个参数错误
'''
class BaZhuoYuRandomAreaClient(ISMS):
    def __init__(self, params, vpsID, serverPath):
        ...
        super(BaZhuoYuRandomAreaClient, self).__init__(params, vpsID, serverPath)
        ...
        if self.cellNum is None:
            ...
            for i in range(10):
                ...
                if res["code"] == 0:
                    ...
                    self.cellNum = str(res["data"][0]["code"] + res["data"][0]["number"])
                    if self.areas.get(res["data"][0]["code"]) is None:
                        errormessage = f"暂时不支持注册该国家的手机号:{res['data'][0]['code']}"
                    else:
                        self.data["area"] = self.areas[res["data"][0]["code"]]
                        self.area = self.areas[res["data"][0]["code"]]
